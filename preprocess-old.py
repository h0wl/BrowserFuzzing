...
def xhelp():
    ...
    print("[*] Example : python preprocess.py -d '320andup-master'")
    sys.exit(1)
	
# 输入仓库路径，返回文件的绝对路径(二维list)，list[x][0]是文件名，list[x][1]是文件路径
def get_html_css_js_files(repo_path):
    ...
    if os.path.isdir(repo_path):  # 判断repo_path是否是一个文件夹
        for root, dirs, files in os.walk(repo_path): #遍历目录
            for file in files:
                r = re.compile(html_pattern).findall(file) #正则判断是否为html文件
                if len(r) != 0:
                    abs_file_path = os.path.join(root, file) #文件路径
